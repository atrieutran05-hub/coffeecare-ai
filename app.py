...
if camera_image is not None:
    ...
    # Lọc bỏ số thứ tự ở đầu tên class (VD: "0 Rust" -> "Rust")
    disease_name = predicted_class.split(" ", 1)[1] if " " in predicted_class else predicted_class

    st.write("---")
    st.subheader(f"🔍 Kết quả phân tích (Độ tin cậy: {confidence_score:.1f}%)")
